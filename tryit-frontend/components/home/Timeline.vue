<template>
  ...
  <v-dialog v-model="isVisible" max-width="600px" fullscreen>
    ...
    <v-alert v-if="$fetchState.error" type="error">
      <v-row>
        ...
        <v-col cols="4">
          ...
          <v-btn color="secondary" @click="hideDialog">SALIR
          </v-btn>
        </v-col>
      </v-row>
    </v-alert>
    <v-card>
      <v-toolbar color="primary" dark flat>
        ...
        <v-progress-circular v-if="$fetchState.pending" :width="3" color="teal lighten-4"
        ...
        <v-tab-item v-for="(post) in posts" :key="post.day">
          ...
          <v-timeline>
            <v-slide-x-reverse-transition group hide-on-leave>
              <v-timeline-item v-for="(event, index) in posts[tab].events" :key="event.id" :color="event.color"
                :icon="event.icon" large fill-dot>
                <template v-slot:opposite>
                  <span :class="`headline font-weight-bold ${event.color}--text text-h4`">
                    De {{ dateToTimeStr(event.start_date) }} a {{ dateToTimeStr(event.end_date) }}
                  </span>
                </template>
                <v-card color="primary" class="elevation-2">
                  <v-img max-height="2vh" src="/img/tryit_background1.png">
                  ...
                  <v-spacer />
                  <v-card-subtitle class="responsive-txt-subtitle-2 white--text text-left"> {{ getSubtitle(event) }}
                  </v-card-subtitle>

                  <v-card-actions>
                    <v-dialog v-model="show[index]" max-width="80%">
                      <template v-slot:activator="{ on, attrs }">
                        <v-row align="center" justify="space-around">
                          <v-btn dark icon v-bind="attrs" large color="secondary" v-on="on" @click="showInfo(index)">
                      ...
                      <v-card class="mx-auto">
                        ...
                        <v-card-actions v-if="event.start_date">
                          ...
                          <v-btn icon @click="show_time = !show_time">
                        ...
                        <v-expand-transition>
                          <div v-show="show_time">
                            ...
                            <v-card-text>
                              La actividad comienza el {{ dateToStr(event.start_date) }} y termina el {{
                              dateToStr(event.end_date) }}
                            </v-card-text>
                          </div>
                        </v-expand-transition>


                        <v-card-actions v-if="event.desc">
                          <v-btn color="primary" text>
                            Sobre la Actividad
                          </v-btn>
                        ...
                        <v-card-actions v-if="event.speakers">
                          <v-btn color="primary" text>
                            Ponente/s
                          </v-btn>
                          ...
                          <v-btn icon @click="show_speakers = !show_speakers">
                        ...
                        <v-expand-transition>
                          <div v-show="show_speakers">
                            ...
                            <template>
                              <v-tabs v-model="speaker_tab" grow center-active centered slider-color="secondary" light>
                                
                                <v-tab v-for="speaker in event.speakers" :key="speaker.id">
                            ...
                            <v-tabs-items v-model="speaker_tab">
                              <v-tab-item v-for="speaker in event.speakers" :key="speaker.id">
                                <v-divider>
                                ...
                                <v-card class="mx-auto">
                                  ...
                                  <v-row justify="center">
                                    <v-btn class="mx-4" dark icon v-bind="attrs" large color="secondary" v-on="on"
                                      v-if=speaker.github :href=speaker.github target="_blank">
                                      <v-icon left dark x-large color="grey" class="mx-3">
                                        mdi-github
                                      </v-icon>
                                    </v-btn>
                                    <v-btn class="mx-4" dark icon v-bind="attrs" large color="secondary" v-on="on"
                                      v-if=speaker.twitter :href=speaker.twitter target="_blank">
                                      <v-icon left dark x-large color="grey" class="mx-3">
                                        mdi-twitter
                                      </v-icon>
                                    </v-btn>
                                    <v-btn class="mx-4" dark icon v-bind="attrs" large color="secondary" v-on="on"
                                      v-if=speaker.linkedin :href=speaker.linkedin target="_blank">
                                      <v-icon left dark x-large color="grey" class="mx-3">
                                        mdi-linkedin
                                      </v-icon>
                                    </v-btn>
                                    <v-btn class="mx-4" dark icon v-bind="attrs" large color="secondary" v-on="on"
                                      v-if=speaker.website :href=speaker.website target="_blank">
                                      <v-icon left dark x-large color="grey" class="mx-3">
                                        mdi-web
                                      </v-icon>
                                    </v-btn>
