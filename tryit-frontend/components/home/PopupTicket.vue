  ...
  <div>
    <div v-if="isTicketFormVisible && !this.$auth.loggedIn">
      <v-dialog v-model="isVisible" max-width="610px">
        <v-card>
          <v-card-title style="white-space: normal; word-break: normal; word-spacing: normal;">Para obtener tu entrada, necesitas iniciar sesión o registrarte.</v-card-title>
    ...
    <div v-else-if="isTicketFormVisible && this.$auth.loggedIn">
      <v-dialog v-model="isVisible" max-width="800px">
        <v-alert v-model="good_alert" type="success" close-text="Cerrar" color="green" dismissible>
          ¡Te has registrado correctamente! En unos momentos recibirás un email con tu entrada.
        </v-alert>
        <v-alert v-model="user_already_exists_alert" type="error" close-text="Cerrar" color="red" dismissible>
          ¡Ya te has registrado! No puedes generar una nueva entrada. Si has cometido algún error, contacta a tu
          Delegación de Centro.
        </v-alert>

        <v-alert v-model="must_accept_privacy_terms_alert" type="error" close-text="Cerrar" color="red" dismissible>
          Para poder obtener tu entrada, necesitas aceptar la política de privacidad y protección de datos, y la cesión
          de los derechos de imagen. Las puedes consultar

          <v-btn depressed color="primary" @click="togglePrivacyPolicyVisibility()" plain>
            aquí.
          </v-btn>
        </v-alert>
        <v-alert v-model="error_alert" type="error" close-text="Cerrar" color="red" dismissible>
          Error al registrarte. Habla con la Delegación de Alumnos de Centro.
        </v-alert>
        <v-card color="primary">
          <v-card-title class="white--text primary text-h5">¿Quieres asistir al TryIT! ? ¡Obtén tu
          ...
          <v-card class="pa-5">
            <v-form ref="form">
              ...
              <v-row align="center" justify="space-around" class="mt-5">
                <v-btn rounded x-large dark color="accept" @click="validateAndPost">
                  <v-icon left x-large color="white" class="mx-3">
                    <!-- mdi-information-outline -->
                    mdi-check
                  </v-icon>
                  Conseguir entrada
                ...
                <v-btn dark rounded x-large color="close" @click="hideDialog">
                  <v-icon left dark x-large color="white" class="mx-3">
                    <!-- mdi-information-outline -->
                    mdi-close
                  </v-icon>
                  Cerrar
        ...
        <template>
          <v-dialog v-model="isPrivacyPolicyVisible" max-width="600px">
            <v-card>
              <v-card-title>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn rounded x-large dark color="accept" @click="privacyAndCloseWindow(true)">
                  <v-icon left x-large color="white" class="mx-3">
                    <!-- mdi-information-outline -->
                    mdi-check
                  </v-icon>
                ...
                <v-btn dark rounded x-large color="orange darken-3" @click="privacyAndCloseWindow(false)">
                  <v-icon left dark x-large color="white" class="mx-3">
                    <!-- mdi-information-outline -->
                    mdi-close
                  </v-icon>
            ...
            <v-container>
              <v-card-actions>
                <v-row>
                  <v-col>
                    <v-btn color="primary" small outlined :style="{ left: '50%', transform: 'translateX(-50%)' }"
                      href="https://www.instagram.com/tryit_upm/?hl=es" target="_blank">
                      <v-icon dark dense color="primary" left>
                        <!-- mdi-information-outline -->
                        mdi-instagram
                      </v-icon>
                  ...
                  <v-col>
                    <v-btn color="primary" small outlined :style="{ left: '50%', transform: 'translateX(-50%)' }"
                      href="https://www.twitter.com/tryit_upm/?hl=es" target="_blank">
                      <v-icon dark dense color="primary" left>
                        <!-- mdi-information-outline -->
                        mdi-twitter
                      </v-icon>
